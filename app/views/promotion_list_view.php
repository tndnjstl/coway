  ...
  <div class="content-wrapper">
    <div class="page-inner">
      ...
      <div class="d-flex justify-content-between align-items-center mb-3 flex-wrap gap-2">
        <div>
          <h4 class="fw-bold mb-0">프로모션 관리</h4>
          <nav aria-label="breadcrumb"><ol class="breadcrumb mb-0"><li class="breadcrumb-item active">프로모션 목록</li></ol></nav>
        </div>
        <?php if (is_manager()): ?>
...
document.querySelectorAll('.btn-toggle-promo').forEach(btn => {
  btn.addEventListener('click', async () => {
    ...
    const res = await fetch('/Promotion/toggle', {method:'POST', headers:{'Content-Type':'application/x-www-form-urlencoded'}, body:'uid='+btn.dataset.id});
...
document.querySelectorAll('.btn-delete-promo').forEach(btn => {
  btn.addEventListener('click', async () => {
    ...
    const res = await fetch('/Promotion/delete', {method:'POST', headers:{'Content-Type':'application/x-www-form-urlencoded'}, body:'uid='+btn.dataset.id});
    const d = await res.json();
    if (d.success) location.reload();
  });
});
